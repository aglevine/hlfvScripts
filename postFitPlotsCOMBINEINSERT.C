void postFitPlotsCOMBINEINSERT(TString name="collMass",TString fileDataMuHad0Jet="asdf", TString fileDataMuHad1Jet = "asdf", TString fileDataMuHad2Jet = "asdf", TString fileDataMuEle0Jet = "asdf", TString fileDataMuEle1Jet = "asdf", TString fileDataMuEle2Jet = "asdf", TString file="fit2_signalV5_collMass_type1.root", TString dirInternal="filesBoosted", TString Xaxis="M_{#mu,#tau}_{coll} [GeV]", TString Yaxis="Events / 20 GeV", double xmin=200, double xmax=300, double ymin=-2, double ymax=2, bool setLogY=false, double legx1=0.6, double legy1=0.9, double legx2=0.9, double legy2=0.5, double MAX=-1, double MIN=-1, bool mutau0=true, bool mutau1=true, bool mutau2=true, bool muele0=true, bool muele1=true, bool muele2=true, bool REBIN=true, bool weightSOverSB=true,TString channelName="#mu#tau", double minsubs=-1, double maxsubs=-1){
	...
	TFile *_fileMC= new TFile("CombinedPlots20/"+file);
	TFile *_fileDataMuHad0Jet= new TFile("CombinedPlots20/"+fileDataMuHad0Jet);
        TFile *_fileDataMuHad1Jet= new TFile("CombinedPlots20/"+fileDataMuHad1Jet);
        TFile *_fileDataMuHad2Jet= new TFile("CombinedPlots20/"+fileDataMuHad2Jet);
        TFile *_fileDataMuEle0Jet= new TFile("CombinedPlots20/"+fileDataMuEle0Jet);
        TFile *_fileDataMuEle1Jet= new TFile("CombinedPlots20/"+fileDataMuEle1Jet);
        TFile *_fileDataMuEle2Jet= new TFile("CombinedPlots20/"+fileDataMuEle2Jet);
	...
	TString dirInternalAllMU[3]={dirInternal+"/datacard_gg0_pfMetFixOldjesNewFakeShape",dirInternal+"/datacard_gg1_pfMetFixOldjesNewFakeShape", dirInternal+"/datacard_vbf20_pfMetFixOldjesNewFakeShape"};
        //TString dirInternalAllMU[3]={dirInternal+"/datacard_MuTau_0Jet_new",dirInternal+"/datacard_MuTau_1Jet_new", dirInternal+"/datacard_vbf_20"};
...
//	hSignal->Draw("sameshist");
	...
	entry=leg->AddEntry(hSignal,"LFV Higgs, (B=0.84%)","l");
	...
	RatioError->Draw("samesE2"); //RatioError->SetFillStyle(3002); 
        RatioError->SetFillColorAlpha(kGray+2,0.35); RatioError->SetMarkerSize(0) RatioError->SetLineColor(kBlack);
	Ratio->Draw("sames");
	...
	//SUBTRACTEDMCERROR->SetFillStyle(3004);
        SUBTRACTEDMCERROR->SetFillColorAlpha(kGray+2,0.35);
	SUBTRACTEDMCERROR->SetLineWidth(0);
	...
	hSignalFill->Draw("hist");
        ...
        hSignalFill->GetXaxis()->SetLabelFont(42);
        hSignalFill->GetXaxis()->SetTitleFont(42);
        hSignalFill->GetXaxis()->SetRangeUser(xmin,xmax);
        hSignalFill->SetXTitle(Xaxis);
        hSignalFill->GetXaxis()->SetTitleOffset(0.95);
        hSignalFill->GetXaxis()->SetTitleSize(0.05);
	...
	hSignalFill->GetXaxis()->SetLabelSize(0.03);
	...
	entry=leg2->AddEntry(hSignalFill,"LFV H#rightarrow#mu#tau signal (B=0.84%)","f");
