...
void postFitPlotsEMu(TString name="collMass",TString dirData = "", TString fileData="fit2_signalV5_collMass_type1.root", TString file="fit2_signalV5_collMass_type1.root", TString dir="filesBoosted", TString dirInternalData="filesBoosted", TString dirInternal="ggmutau",TString dirInternalLFV="",bool postfit=true,double blindA=100, double blindB=160, bool blind=false, TString Xaxis="M_{#mu,#tau}_{coll} [GeV]", TString Yaxis="Events / 20 GeV", TString Tex = "",double xmin=200, double xmax=500, double ymin=-2, double ymax=2, int rebinning=1, bool setLogY=false, double legx1=0.6, double legy1=0.9, double legx2=0.9, double legy2=0.5, double MAX=-1, double MIN=-1, double scaleSignal=1.0, bool skipDY=false){
  ...
  //  example_plot( iPeriod, 33 );  // right-aligned
	...
	TString dyfile="/fakes_emu";
	bool ifDYOther=false;
	if(_fileMC->Get(dirInternal+"/zjetsother")) {dyfile="/zjetsother"; ifDYOther=true;}
	...
	hWG_2->Add(hWGStar_2);
        ...
        TH1F *hFAKES_2=_fileMC->Get(dirInternal+"/fakes_emu");
        ...
        TH1F *hLFVVBF126_2=_fileMC->Get(dirInternalLFV+"/LFVVBF"); hLFVVBF126_2->SetName("buh");
	...
	hdata_obs->SetMarkerColor(1);
	hdata_obs->SetLineColor(kBlack);
...
//        hSignal->Draw("sameshist");
   ...
   leg->SetFillStyle(0);
   TLegendEntry *entry;
   entry=leg->AddEntry(hdata_obs,"Data, e#tau_{mu}","p");
