...
//        TH1F *hSMHWWGG126=_fileHWW->Get("extra/SMHWWGG126");
	...
	if(skipDY) getzjets="/fakes";
        std::cout << getzjets << std::endl;

	TH1F *hDY_2=_fileMC->Get(dirInternal+getzjets); 
        hDY_2->SetName("hDY_2");
	if(skipDY) hDY_2->Scale(0);

        TH1F *hWW_2=_fileMC->Get(dirInternal+"/diboson");
 	if (!isVBF){
	...
	cout<<hFAKES_2->GetNbinsX()<<endl;
        ...
        TH1F *hLFVVBF126_2=_fileMC->Get(dirInternalLFV+"/LFVVBF");  
        TH1F *hSMVBF126_2=_fileMC->Get(dirInternal+"/SMVBF126"); 
        TH1F *hLFVGG126_2=_fileMC->Get(dirInternalLFV+"/LFVGG");  
	...
	hSignal->Add(hLFVVBF126);
        ...
        hFAKES->SetFillColor(kMagenta-10); hFAKES->SetLineColor(kMagenta+4); hFAKES->SetLineWidth(1); hFAKES->SetFillStyle(1001);
        hZTauTau->SetFillColor(kOrange-4); hZTauTau->SetLineColor(kOrange+4); hZTauTau->SetLineWidth(1); hZTauTau->SetFillStyle(1001);

        hDY->SetFillColor(kAzure+3); hDY->SetLineColor(kAzure+4); hDY->SetLineWidth(1); hDY->SetFillStyle(1001);
        hWW->SetFillColor(kAzure+3); hWW->SetLineColor(kAzure+3); hWW->SetLineWidth(1); hWW->SetFillStyle(1001);
	if (!isVBF){
        	hTOP->SetFillColor(kGreen-2); hTOP->SetLineColor(kGreen+4); hTOP->SetLineWidth(1); hTOP->SetFillStyle(1001);
	}
        hTT->SetFillColor(kGreen-2); hTT->SetLineColor(kGreen-2); hTT->SetLineWidth(1); hTT->SetFillStyle(1001);

	hSMHIGGS->SetFillColor(kMagenta); hSMHIGGS->SetLineColor(kMagenta); hSMHIGGS->SetLineWidth(1); hSMHIGGS->SetFillStyle(1001);
//        hSMHWWHIGGS->SetFillColor(kViolet-10); hSMHWWHIGGS->SetLineColor(kMagenta+4); hSMHWWHIGGS->SetLineWidth(1);
	 hSMHWWHIGGS->SetFillColor(kMagenta); hSMHWWHIGGS->SetLineColor(kMagenta+1); hSMHWWHIGGS->SetLineWidth(1); hSMHWWHIGGS->SetFillStyle(1001);
...
// PLOT
	...
	fullMC2->Add(hSMHIGGS);  fullMC2->Add(hSMHWWHIGGS);
...
//        hSignal->Draw("sameshist");
   ...
   entry=leg->AddEntry(hFAKES,"Misidentified #tau","f");
	...
	if(postfit) entry=leg->AddEntry(hSignal,"LFV Higgs (B=1%)","l");
