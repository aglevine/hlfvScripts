...
if presel:
	...
	if sysname == "jes":
		savedir = "jesnonesignalFeb26_01JetFix/"
		esplusdir = "jesplussignalFeb26_01JetFix/"
		esminusdir = "jesminussignalFeb26_01JetFix/" 
	elif sysname == "tes":
		savedir = "signal_Feb26_01JetFixNew/"
		esplusdir = "tesup_signal_March9/"
		esminusdir = "tesdown_signal_March9/"
	elif sysname == "ues":
		savedir = "uesnonesignalApr26/"
		esplusdir = "uesplussignalApr26/"
		esminusdir = "uesminussignalApr26/"
	elif sysname == "fakes":
		savedir = "jesnonesignalFeb26_01JetFix/"
		esplusdir = "fakeShapeShiftUp_May7/"
		fakesdowndir = "fakeShapeShiftDown_May7/"
	else:
		print "invalid sysname"
		sys.exit()
savedir = "jesnonesignalFeb26_01JetFix/"
...
hgglfvesplus_ntuple_file = ROOT.TFile(esplusdir+hgglfv_ntuple_file_str)
hgglfvesminus_ntuple_file = ROOT.TFile(esminusdir+hgglfv_ntuple_file_str)
hvbflfv_ntuple_file = ROOT.TFile(savedir+hvbflfv_ntuple_file_str)
hvbflfvesplus_ntuple_file = ROOT.TFile(esplusdir+hvbflfv_ntuple_file_str)
hvbflfvesminus_ntuple_file = ROOT.TFile(esminusdir+hvbflfv_ntuple_file_str)
...
if zjetsEmbed:
	...
	#	dy4other = dy4other_ntuple_file.Get(ntuple_spot+"/"+var).Clone()
if massCmp == True:
	hvbflfvCollMass = hvbflfv_ntuple_file.Get(ntuple_spot+"/collMass_type1").Clone()
        hgglfvCollMass = hgglfv_ntuple_file.Get(ntuple_spot+"/collMass_type1").Clone()
	#hvbflfvCollMass = hvbflfv_ntuple_file.Get(ntuple_spot+"/jetVeto30").Clone()
        #hgglfvCollMass = hgglfv_ntuple_file.Get(ntuple_spot+"/jetVeto30").Clone()
        #hvbflfvCollMass = hvbflfv_ntuple_file.Get(ntuple_spot+"/jetVeto30").Clone()
        #hgglfvCollMass = hgglfv_ntuple_file.Get(ntuple_spot+"/jetVeto30").Clone()
	if sysname  == "jes":
		hvbflfvesminus = hvbflfvesminus_ntuple_file.Get(ntuple_spot+"/collMass_jes_minus").Clone()
		hvbflfvesplus = hvbflfvesplus_ntuple_file.Get(ntuple_spot+"/collMass_jes_plus").Clone()
        	hgglfvesminus = hgglfvesminus_ntuple_file.Get(ntuple_spot+"/collMass_jes_minus").Clone()
        	hgglfvesplus = hgglfvesplus_ntuple_file.Get(ntuple_spot+"/collMass_jes_plus").Clone()

                #hvbflfvesminus = hvbflfvesminus_ntuple_file.Get(ntuple_spot+"/tMtToPfMet_jes_minus").Clone()
                #hvbflfvesplus = hvbflfvesplus_ntuple_file.Get(ntuple_spot+"/tMtToPfMet_jes_plus").Clone()
                #hgglfvesminus = hgglfvesminus_ntuple_file.Get(ntuple_spot+"/tMtToPfMet_jes_minus").Clone()
                #hgglfvesplus = hgglfvesplus_ntuple_file.Get(ntuple_spot+"/tMtToPfMet_jes_plus").Clone()

                #hvbflfvesminus = hvbflfvesminus_ntuple_file.Get(ntuple_spot+"/tMtToPfMet_Ty1").Clone()
                #hvbflfvesplus = hvbflfvesplus_ntuple_file.Get(ntuple_spot+"/tMtToPfMet_Ty1").Clone()
                #hgglfvesminus = hgglfvesminus_ntuple_file.Get(ntuple_spot+"/tMtToPfMet_Ty1").Clone()
                #hgglfvesplus = hgglfvesplus_ntuple_file.Get(ntuple_spot+"/tMtToPfMet_Ty1").Clone()

                #hvbflfvesminus = hvbflfvesminus_ntuple_file.Get(ntuple_spot+"/jetVeto30jes_minus").Clone()
                #hvbflfvesplus = hvbflfvesplus_ntuple_file.Get(ntuple_spot+"/jetVeto30jes_plus").Clone()
                #hgglfvesminus = hgglfvesminus_ntuple_file.Get(ntuple_spot+"/jetVeto30jes_minus").Clone()
                #hgglfvesplus = hgglfvesplus_ntuple_file.Get(ntuple_spot+"/jetVeto30jes_plus").Clone()
	elif sysname == "tes":
                hvbflfvesminus = hvbflfvesminus_ntuple_file.Get(ntuple_spot+"/collMass_type1").Clone()
                hvbflfvesplus = hvbflfvesplus_ntuple_file.Get(ntuple_spot+"/collMass_type1").Clone()
                hgglfvesminus = hgglfvesminus_ntuple_file.Get(ntuple_spot+"/collMass_type1").Clone()
                hgglfvesplus = hgglfvesplus_ntuple_file.Get(ntuple_spot+"/collMass_type1").Clone()
	elif sysname == "ues":
                hvbflfvesminus = hvbflfvesminus_ntuple_file.Get(ntuple_spot+"/collMass_type1_ues_minus").Clone()
                hvbflfvesplus = hvbflfvesplus_ntuple_file.Get(ntuple_spot+"/collMass_type1_ues_plus").Clone()
                hgglfvesminus = hgglfvesminus_ntuple_file.Get(ntuple_spot+"/collMass_type1_ues_minus").Clone()
                hgglfvesplus = hgglfvesplus_ntuple_file.Get(ntuple_spot+"/collMass_type1_ues_plus").Clone()
	if ntuple_spot == "gg0" or ntuple_spot == "gg1":
		hesnone = hgglfvCollMass.Clone()
		hesplus = hgglfvesplus.Clone()
		hesminus = hgglfvesminus.Clone()
	else:
                hesnone = hvbflfvCollMass.Clone()
                hesplus = hvbflfvesplus.Clone()
                hesminus = hvbflfvesminus.Clone()		
...
if massCmp == True:
	if ntuple_spot == "gg1" or ntuple_spot == "gg0":
		massCmpScale = hgglfv_norm
	else:
		massCmpScale = hvbflfv_norm
	hesnone.Scale(massCmpScale)
	hesplus.Scale(massCmpScale)
	hesminus.Scale(massCmpScale)
...
if massCmp == True:
	hesnone.Rebin(binwidth)
	hesplus.Rebin(binwidth)
	hesminus.Rebin(binwidth)
...
if prelimColors == True:
	...
	if massCmp == True:
		hesnone.SetLineColor(ROOT.EColor.kBlack)
		hesnone.SetMarkerSize(0)
		hesnone.SetLineWidth(3)
		hesminus.SetLineColor(ROOT.EColor.kBlue)
		hesminus.SetMarkerSize(0)
		hesminus.SetLineWidth(3)
		hesplus.SetLineColor(ROOT.EColor.kRed)
		hesplus.SetMarkerSize(0)
		hesplus.SetLineWidth(3)
...
if massCmp == True:
	...
	if ntuple_spot == "gg0" or ntuple_spot == "gg1":
		legendmassCmp.AddEntry(hesnone,'LFV GG Higgs')
	else:
		legendmassCmp.AddEntry(hesnone,'LFV VBF Higgs')
	if sysname == "jes":
		esplusStr = "JES + 1 #sigma"
# %.1f%% change in yield"%((hesplus.Integral()-hesnone.Integral())/hesnone.Integral()*100)
        	esminusStr = "JES - 1 #sigma"
#%.1f%% change in yield"%((hesminus.Integral()-hesnone.Integral())/hesnone.Integral()*100)
	elif sysname == "tes":
                esplusStr = "TES + 1 #sigma"
	#%.1f%% change in yield"%((hesplus.Integral()-hesnone.Integral())/hesnone.Integral()*100)
                esminusStr = "TES - 1 #sigma"
	#%.1f%% change in yield"%((hesminus.Integral()-hesnone.Integral())/hesnone.Integral()*100)
	elif sysname == "ues":
		esplusStr = "MET + 1 #sigma"
	#%.1f%% change in yield"%((hesplus.Integral()-hesnone.Integral())/hesnone.Integral()*100)
                esminusStr = "MET - 1 #sigma"
	#%.1f%% change in yield"%((hesminus.Integral()-hesnone.Integral())/hesnone.Integral()*100)

	legendmassCmp.AddEntry(hesplus,esplusStr)
	legendmassCmp.AddEntry(hesminus,esminusStr)
...
if massCmp == True:
	maxmassCmp = max(hesnone.GetMaximum(),hesminus.GetMaximum(),hesplus.GetMaximum())
	hesnone.SetMaximum(maxmassCmp*1.2)
print ":((((((((((((((((((((("
print maxdata
...
if massCmp == False:
	...
	hesplus.Scale(hesnone.Integral()/hesplus.Integral())
	hesminus.Scale(hesnone.Integral()/hesminus.Integral())
	hesnone.Draw('hist')
	hesnone.SetTitle("")
	hesplus.Draw('sameshist')
	hesminus.Draw('sameshist')
if systematics == True:
...
if massCmp:
	hesnone.GetXaxis().SetTitle(xlabel)
	hesnone.GetXaxis().SetNdivisions(505)
	hesnone.GetXaxis().SetTitleOffset(1.2)
	hesnone.GetXaxis().SetLabelSize(0.035)
	hesnone.GetXaxis().SetTitleSize(0.05)
	print ylabel
	hesnone.GetYaxis().SetTitle(ylabel)
	hesnone.GetYaxis().SetTitleOffset(1.3)
	hesnone.GetYaxis().SetLabelSize(0.035)
	hesnone.GetYaxis().SetTitleSize(0.05)
#else:
	#if shape_norm == False:
...
if massCmp == True:
        p_ratioSys.cd()
        p_ratioSys.Draw()
        ratioUp = hesplus.Clone()
        ratioDown = hesminus.Clone()
        ratioUp.Divide(hesnone)
        ratioDown.Divide(hesnone)
...
if massCmp == True:
		canvas.SaveAs(savedir +"muhad_masses_June5NormRatios"+ntuple_spot+"_"+sysname+"_signal.pdf")
if fakeRate == False:
