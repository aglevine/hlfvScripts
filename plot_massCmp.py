...
if zjetsEmbed:
	...
	#	dy4other = dy4other_ntuple_file.Get(ntuple_spot+"/"+var).Clone()
if massCmp == True:
	hvbflfvVisMass = hvbflfv_ntuple_file.Get(ntuple_spot+"/m_t_Mass").Clone()
...
if massCmp == True:
	hvbflfvVisMass.Scale(hvbflfv_norm)
	hvbflfvCollMass.Scale(hvbflfv_norm)
...
if massCmp == True:
	hvbflfvVisMass.Rebin(binwidth)
	hvbflfvCollMass.Rebin(binwidth)
...
if prelimColors == True:
	...
	if massCmp == True:
		hvbflfvVisMass.SetLineColor(ROOT.EColor.kRed)
		hvbflfvVisMass.SetMarkerSize(0)
		hvbflfvVisMass.SetLineWidth(3)
		hvbflfvCollMass.SetLineColor(ROOT.EColor.kBlack)
		hvbflfvCollMass.SetMarkerSize(0)
		hvbflfvCollMass.SetLineWidth(3)
...
if massCmp == True:
	legendmassCmp = ROOT.TLegend(0.2,0.8,0.9,1.0,' ','brNDC')
	legendmassCmp.AddEntry(hvbflfvVisMass,'LFV VBF Higgs #mu#tau Mass')
	legendmassCmp.AddEntry(hvbflfvCollMass,'LFV VBF Higgs Collinear Mass')
...
if massCmp == True:
	maxhvbflfvVisMass = hvbflfvVisMass.GetMaximum()
	maxhvbflfvCollMass = hvbflfvCollMass.GetMaximum()
	maxmassCmp = max(maxhvbflfvVisMass,maxhvbflfvCollMass)
	hvbflfvVisMass.SetMaximum(maxmassCmp*1.2)
print ":((((((((((((((((((((("
print maxdata
...
if massCmp == False:
	...
	hvbflfvVisMass.Draw('hist')
	hvbflfvVisMass.SetTitle("")
	hvbflfvCollMass.Draw('sameshist')
if systematics == True:
...
if massCmp:
	legendmassCmp.SetTextSize(0.03)
if massCmp == False:
	legend.Draw('sames')
else:
	legendmassCmp.Draw('sames')
if massCmp:
	hvbflfvVisMass.GetXaxis().SetTitle("[GeV]")
	hvbflfvVisMass.GetXaxis().SetNdivisions(505)
	hvbflfvVisMass.GetXaxis().SetTitleOffset(1.0)
	hvbflfvVisMass.GetXaxis().SetLabelSize(0.035)
	print ylabel
	hvbflfvVisMass.GetYaxis().SetTitle(ylabel)
	hvbflfvVisMass.GetYaxis().SetTitleOffset(1.3)
	hvbflfvVisMass.GetYaxis().SetLabelSize(0.035)
...
MassVisMean = hvbflfvVisMass.GetMean()
MassVisSigma = hvbflfvVisMass.GetRMS()
MassColMean = hvbflfvCollMass.GetMean()
MassColSigma = hvbflfvCollMass.GetRMS()

latexStats = ROOT.TLatex()
latexStats.SetNDC()
latexStats.SetTextSize(0.04)
latexStats.SetTextAlign(36)
latexStatsStr = "#barm_{vis} = %.1f GeV"%(MassVisMean)
latexStats.DrawLatex(0.52,0.7,latexStatsStr)
latexStats.SetTextAlign(36)
latexStats.DrawLatex(0.42, 0.6, "#sigma_{vis} = %.3f"%(MassVisSigma/MassVisMean))
latexStats.DrawLatex(0.52, 0.5, "#barm_{col} = %.1f GeV"%(MassColMean))
latexStats.DrawLatex(0.42, 0.4, "#sigma_{col} = %.3f"%(MassColSigma/MassColMean))
...
if massCmp == True:
		canvas.SaveAs(savedir +"masses_muhad_VBF_After_Presel_signal.pdf")
if fakeRate == False:
