...
*/
//mu/e then mu/tau
        char *channels[ny]={"H#rightarrowe#tau", "e#tau_{h}, 2 Jets","e#tau_{h}, 1 Jet", "e#tau_{h}, 0 Jets","e#tau_{#mu}, 2 Jets","e#tau_{#mu}, 1 Jet", "e#tau_{#mu}, 0 Jets"};

        double SIGMA[7]=  { 0.383657, 1.96911, 1.47085, 1.27155, 0.809167, 0.783257, 0.833083};
        double ERR2DOWN[7]={ 0.3951, 1.8015, 1.5259, 1.3191, 0.7341, 0.8006, 0.8642};
        double ERRDOWN[7]= {0.5331, 2.5330, 2.0506, 1.7636, 1.0369, 1.0789, 1.1555};
        double MEDIAN[7]= { 0.740234, 3.70312, 2.58594, 2.447656, 1.5859, 1.5352, 1.6328};
        double ERRUP[7]={   1.0727, 6.2285, 4.1353, 3.5551, 2.5342, 2.2266, 2.3422};
        double ERR2UP[7]=  {  1.4757, 10.0578, 5.7453, 4.9538, 3.9823, 3.1517, 3.2541};
        double OBSERVED[7]=  {0.7872,3.0621,3.5842,4.5510,1.4907,0.9408,1.8277};

        //        double OBSERVED[7]=  { -10,-10,-10,-10,-10,-10,-10};
 
        double R2DOWN[7]={ 0.3889, 1.7503, 1.3687, 1.3108, 0.7341, 0.8006, 0.8642};
        double RDOWN[7]={0.5234, 2.4445, 1.8347, 1.7616, 1.0369, 1.0789, 1.1555};
        double RUP[7]={1.0559, 5.9763, 3.7198, 3.5526, 2.5342, 2.2266, 2.3422}; 
        double R2UP[7]={1.4527, 9.5680, 5.1927, 4.9357, 3.9823, 3.1517, 3.2541};         
	...
	for(int i=0; i<7; i++) {
		...
		channels2[i]=Form("#splitline{%s}{ #scale[0.8]{ #splitline{%1.2f%% (exp.)}{%1.2f%% (obs.)}}}",channels[i],MEDIAN[i],OBSERVED[i]);
		  //channels2[i]=Form("#splitline{%s}{#scale[0.8]{ #splitline{%1.2f%% (exp.)}{" "}}}",channels[i],MEDIAN[i]);
	...
	h->SetXTitle("95% CL limit on B(H#rightarrowe#tau), %");
	...
	PLOTMU->SaveAs("LimitsETau.png");
	PLOTMU->SaveAs("LimitsETau.pdf");
	//cmsPrelim(19700);
